...
def createSuitReviveTrack(suit, toon, battle, npcs = []):
    ...
    suitTrack.append(Parallel(Func(suit.showHpText, "0.5x MAX HP MULTIPLIER", 3, openEnded=0),Sequence(Wait(0.9), Func(suit.showHpText, "1.5x DMG MULTIPLIER", 4, openEnded=0))))
    suitTrack.append(ActorInterval(suit, 'slip-forward'))
    suitTrack.append(Func(suit.loop, 'neutral'))
    suitTrack.append(Func(suit.setHP, int(suit.maxHP / 2)))
    suitTrack.append(Func(suit.setMHP, int(suit.maxHP / 2)))
...
def createSuitDeathTrack(suit, toon, battle, npcs = [], headless = False):
    ...
    suitPos, suitHpr = battle.getActorPosHpr(suit)
    removeTrainTrack(suit, battle, suitTrack)
    deathSuit = suit.getLoseActor(headless)
