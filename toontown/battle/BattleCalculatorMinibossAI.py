...
class BattleCalculatorMinibossAI(BattleCalculatorAI.BattleCalculatorAI):
    notify = DirectNotifyGlobal.directNotify.newCategory('BattleCalculatorMinibossAI')
    ...
    def __init__(self, battle, tutorialFlag=0):
        BattleCalculatorAI.BattleCalculatorAI.__init__(self, battle, tutorialFlag)
        self.notify.debug("USING MINIBOSS CALCULATOR")
        self.battle = battle
        self.SuitAttackers = {}
    ...
    def __applySuitAttackDamages(self, attackIndex):
        ...
        if self.APPLY_HEALTH_ADJUSTMENTS:
            for t in self.battle.activeToons:
                self.notify.debug('TESTING SHIT HERE')
                self.notify.debug('turns %i' % self.TurnsElapsed)
